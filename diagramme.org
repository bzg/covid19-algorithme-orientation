...
elseif (SI (fièvre et PAS toux) OU (toux ET douleurs) OU (toux ET anosmie) OU (fièvre ET diarrhée)) then
        if (SI un facteur de gravité majeur) then
           :FIN5;
           stop
        elseif (SI 0 facteur pronostique) then
               if (0 facteur de gravité mineur) then
                  if (NI moins de 15 ans NI plus de 49 ans) then
                     :FIN2;
                     stop
                  else
                    :FIN3;
...
elseif (SI pas de fièvre et (toux OU douleurs OU anosmie)) then
       if (0 facteur de gravité mineur) then
          :FIN7; 
          stop
       else
       ...
       if (SI NI toux NI douleurs NI anosmie NI fièvre) then
         :FIN9;
         stop
       endif
endif
