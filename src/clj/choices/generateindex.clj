...
(def website-contents (md/md->hiccup (slurp "website-contents.md")))
...
(defn head []
  [:head
   [:title (:title website-meta)]
   [:meta {:charset "utf-8"}]
   [:meta {:name "keywords" :content (:keywords website-meta)}]
   [:meta {:name "description" :content (:description website-meta)}]
   [:meta {:name "viewport" :content "width=device-width, initial-scale=1, shrink-to-fit=yes"}]
   [:link {:rel "canonical" :href (:url website-meta)}]
   [:meta {:property "og:locale", :content "fr_FR"}]
   [:meta {:property "og:type", :content "website-meta"}]
   [:meta {:property "og:title", :content (:index-title website-meta)}]
   [:meta {:property "og:url", :content (:url website-meta)}]
   [:meta {:property "og:site_name", :content (:index-title website-meta)}]
   [:meta {:property "og:image", :content (:image-url website-meta)}]
   [:meta {:name "twitter:card", :content "summary_large_image"}]
   [:meta {:name "twitter:title", :content (:index-title website-meta)}]
    ...
    [:div.columns
     [:div.column.is-offset-2.is-4
      ...
      [:ul.footer__social
       ...
       [:li [:a {:href (str "mailto:" (:email website-meta)), :title "Contacter par email"}
...
(defn default []
  (let [title    (:index-title website-meta)
        subtitle (:index-subtitle website-meta)]
    (h/html5
     {:lang "fr"}
     (head)
     [:body
      ...
      [:nav.navbar {:role "navigation" :aria-label "main navigation"}
       [:div.navbar-brand
        [:a.navbar-item {:href (:url website-meta)}
         [:img {:src    "images/logo-marianne.svg"
                :alt    "Logo Marianne"
                :width  "120"
                :height "100"}
          (:url website-meta) ;;FIXME
          ]]]
       [:div.navbar-end
        [:div.navbar-menu
         [:a.navbar-item
          {:href  (:url website-meta)
           :title "Site principal"} "Site principal"]]]]
      [:section.hero
       [:div.hero-body
        [:div.container
